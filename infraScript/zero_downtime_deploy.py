...
class ServiceManager:
    # 초기화 함수
    def __init__(self, socat_port: int = 8001, sleep_duration: int = 3) -> None:
        self.socat_port: int = socat_port
        self.sleep_duration: int = sleep_duration
        self.services: Dict[str, int] = {
            'here_is_paw_api_gateway_1': 8002,
            'here_is_paw_api_gateway_2': 8003
        }
    ...
    def _find_current_service(self) -> None:
        ...
        if not current_service:
            self.current_name, self.current_port = 'here_is_paw_api_gateway_2', self.services['here_is_paw_api_gateway_2']
        else:
    ...
    def _cleanup_dangling_images(self) -> None:
        ...
        # 먼저 이미지 목록 확인
        cmd_list = "docker images | grep '.*/here-is-paw/here_is_paw_api_gateway' | grep '<none>'"
    ...
    def _run_container(self, name: str, port: int) -> None:
        os.system(
            f"docker run -d --name={name} --restart unless-stopped -p {port}:8001 -e TZ=Asia/Seoul -v /dockerProjects/here_is_paw_msa/api_gateway/volumes/gen:/gen --pull always ghcr.io/here-is-paw/here_is_paw_api_gateway")
##
